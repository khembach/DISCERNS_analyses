...
The strandedness iss "reverse" becasuse the libraries were prepared with the Illumina TruSeq protocol. 

### SRR2002765

This the control sample from WT Hela cells. We do not expect to find many novel splicing event.
```{r SRR2002765}
...
if(!file.exists(here("Rmd/rds/novel_control.rds"))){
  ...
  print(Sys.time() - start_time)  ## 39 minutes
...
source("/home/Shared/kathi/exondiscovery/R/exon_from_sj.R")
source("/home/Shared/kathi/exondiscovery/R/junction_reads.R")
source("/home/Shared/kathi/exondiscovery/R/import_novel_sj_reads.R")
source("/home/Shared/kathi/exondiscovery/R/prepare_annotation.R")
source("/home/Shared/kathi/exondiscovery/R/exon_discovery_utils.R")
...
min_intron_size = 21
cores = 20
tile_width <- 1e8 ## ~ 3 million reads per tile

# Which one is faster?
gc()
system.time(
      reads1 <- import_novel_sj_reads_parallel(bam, yield_size = 1000000, 
                                               sj_unann, cores = cores)) # elapsed 324
gc()
system.time(reads2 <- do.call("c", reads1)) ## 195
gc()
system.time( reads <- import_novel_sj_reads(bam, sj_unann) ) ## 840


gc()
system.time(
      reads1 <- import_novel_sj_reads_parallel(bam, yield_size = 1000000, 
                                               sj_unann, cores = cores)) ##431
gc()
system.time(reads2 <- do.call("c", reads1)) ## 188
gc()
system.time( reads <- import_novel_sj_reads(bam, sj_unann) ) ## 800


gc()
system.time(
      reads1 <- import_novel_sj_reads_parallel(bam, yield_size = 1000000, 
                                               sj_unann, cores = cores)) ## 558
gc()
system.time(reads2 <- do.call("c", reads1)) ## 189
gc()
system.time( reads <- import_novel_sj_reads(bam, sj_unann) ) ## 890

```

```{r eval = FALSE, include = FALSE}
  sj_unann <- data.frame(sj_unann, stringsAsFactors = TRUE)
   exons_filtered <- exons[seqnames(exons) %in% sj_unann$seqnames]

gc()
system.time(
   touching <- mcmapply(sj_touching_exon, as.character(sj_unann$seqnames), 
                         sj_unann$start, sj_unann$end, sj_unann$strand,
                       MoreArgs = list(exons = exons_filtered),
                       mc.cores = cores, USE.NAMES = FALSE) ##210
)
gc()
system.time(
    touching1 <- mclapply(seq_along(GRanges(sj_unann)), function(i) 
      sj_touching_exon_gr(GRanges(sj_unann)[i], exons = exons_filtered), 
      mc.cores = cores) ) ## 199
sj_unann <- GRanges(sj_unann)
gc()
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(as.character(seqnames(sj_unann[i])), 
                          start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
                                exons = exons_filtered), mc.cores = cores)) ## 111
gc()
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
                          ex = exons_filtered[seqnames(exons_filtered) == 
                                                as.character(seqnames(sj_unann[i]))]), 
      mc.cores = cores)) ## 117
...
system.time(
   touching <- mcmapply(sj_touching_exon, as.character(sj_unann$seqnames), 
                         sj_unann$start, sj_unann$end, sj_unann$strand,
                       MoreArgs = list(exons = exons_filtered),
                       mc.cores = cores, USE.NAMES = FALSE) ) ## 122
gc()
system.time(
    touching1 <- mclapply(seq_along(GRanges(sj_unann)), function(i) 
      sj_touching_exon_gr(GRanges(sj_unann)[i], exons = exons_filtered), 
...
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(as.character(seqnames(sj_unann[i])), 
                          start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
                                exons = exons_filtered), mc.cores = cores)) ## 116
gc()
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
...
system.time(
   touching <- mcmapply(sj_touching_exon, as.character(sj_unann$seqnames), 
                         sj_unann$start, sj_unann$end, sj_unann$strand,
                       MoreArgs = list(exons = exons_filtered),
                       mc.cores = cores, USE.NAMES = FALSE) )## 119; and 139
gc()
system.time(
    touching1 <- mclapply(seq_along(GRanges(sj_unann)), function(i) 
      sj_touching_exon_gr(GRanges(sj_unann)[i], exons = exons_filtered), 
      mc.cores = cores)) ##  202; and 203
gc()
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(as.character(seqnames(sj_unann[i])), 
                          start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
                                exons = exons_filtered), mc.cores = cores)) ## 114
gc()
system.time(
    touching1 <- mclapply(seq_along(sj_unann), function(i) 
      sj_touching_exon_gr(start(sj_unann[i]), end(sj_unann[i]), strand(sj_unann[i]), 
...
if(!file.exists(here("Rmd/rds/novel_kd.rds"))){
  ...
  novel_kd <- find_novel_exons(sj_filename = sj, annotation = anno,
                               ...
                               yield_size = 10000000, read_length = 100,
                               cores = 20, tile_width = 1e8)
  print(Sys.time() - start_time) ## 43 mins  ## now 53
...
```{r}
ling <- read.table(here("original_paper", "human_ce_location_GRCh38_gene_name.txt"),
                   header = TRUE)
# convert to GRanges
ling <- GRanges(ling$location, gene = ling$gene)
...
export(GRanges(novel_control), here("results", "control_exondiscovery_unfiltered.bed"), format = "BED")

novel_kd$name <- "cassette"
novel_kd$name[is.na(novel_kd$lend)] <- "5'_terminal"
novel_kd$name[is.na(novel_kd$rstart)] <- "3'_terminal"
export(GRanges(novel_kd), here("results", "kd_exondiscovery_unfiltered.bed"), format = "BED")
```
...
export(gtf_control, 
       here("results", "control_exondiscovery_unfiltered.gtf"), format = "GTF")
export(gtf_kd, 
       here("results", "kd_exondiscovery_unfiltered.gtf"), format = "GTF")
```

We sort and index the gtf files for visualization in IGV. For this, we use the [gff3sort.pl](https://github.com/billzt/gff3sort) and igvtools for indexing
```{r, eval = FALSE}
cmd <-  paste0("gff3sort.pl ", 
               here("results", "control_exondiscovery_unfiltered.gtf"), 
               " > ", 
               here("results", "control_exondiscovery_unfiltered.sorted.gtf"))
system(command = cmd)
cmd <-  paste0("gff3sort.pl ", 
               here("results", "kd_exondiscovery_unfiltered.gtf"), 
               " > ", 
               here("results", "kd_exondiscovery_unfiltered.sorted.gtf"))
system(command = cmd)
```
...
How many novel exons per genes?
For this analysis, we need the extended GTF file. Since there are still issues with the extension. I will skip this for now.
TODO: do this analysis once the function is fixed!!
...
We create a MA-plot from the list of novel predictions. Each novel exon is one point and we count the number of reads that overlap with the novel exon in both samples. 
Or what about the number of supporting reads in each sample?
...
```{r}
## Genereate a key for each prediction
...
Why do we have so many predictions specific for one dataset but with nearly equal amounts of reads in the other sample?
